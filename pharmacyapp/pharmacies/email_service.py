...
class EmailService:
    ...
    def send_order_success_email(order):
        try:
            ...
            if hasattr(order, 'online_order'):
                shipping_method = order.online_order.shipping_method
                
                if hasattr(order.online_order, 'ship_info'):
            ...
            if shipping_method == "home_delivery":
                shipping_html = f"""
                    ...
                    <p><strong>Ghi chú:</strong> {shipping_note}</p>
                    </div>
                """
                ...
                <div class="container">
                    ...
                    <div class="content">
                        ...
                        <div class="order-info">
                            ...
                            <p><strong>Trạng thái:</strong> {"Đang chờ xử lý" if order.status == "pending" else "Đã xác nhận"}</p>
                            <p><strong>Phương thức nhận hàng:</strong> {"Giao hàng tại nhà" if shipping_method == "home_delivery" else "Nhận hàng tại cửa hàng"}</p>
                            <p><strong>Phương thức thanh toán:</strong> {"Thanh toán khi nhận hàng" if order.paymentMethod == "cod" else "Thanh toán qua VNPay"}</p>
                        </div>
            ...
            html_content += f"""
                        ...
                        <p>Trân trọng,<br><strong>PharmacyApp</strong></p>
                    </div>
                    
                    <div class="footer">
                        <p>&copy; 2025 PharmacyApp.</p>
                    </div>
                </div>
            </body>
            </html>
